...
def run():
    ...
    [
        var.tasks.append(get_full_task(personal_config))
    ...
    statuses: list[DeviceStatus] = [
        DeviceStatus(_device, None, None, None, False) for _device in devices
    ]
        ...
        for status in statuses:
            if status.process != None:
                if not status.process.is_alive():
                    ...
                    status.device.logger.debug(
                        f"TaskProcess {task_hash} ended, ready to clear"
                    )
                    running_result[task_hash] = status.process_shared_status.get(
                        "result", None
                    )
                    status.process = None
        ...
        def running_devices_count():
            return len(
                [
                    ...
                    if _status.process != None and not _status.finished
                ]
            )
        ...
        for status in statuses:
            ...
            if status.process == None:
                ...
                if _running_devices_count < device_count_limit:
                    logger.debug(
                        f"{_running_devices_count} devices is running. OK for this"
                    )
                    ...
                    if var.tasks:
                        distribute_task = (
                            [
                                ...
                                if task.get("device") == status.device.alias
                            ]
                            or [
                                task for task in var.tasks if task.get("device") is None
                            ]
                            or [None]
                        )[0]
                        ...
                        if distribute_task:
                            ...
                            logger.debug(
                                f'Ready to start a task process(task={distribute_task["hash"]})'
                            )
                            process.start()
                        else:
                            no_task()
        ...
        def append():
            if final_task_config.get("enable", True):
                final_maatasks.append(
                    {"task_name": final_task_name, "task_config": final_task_config}
                )
