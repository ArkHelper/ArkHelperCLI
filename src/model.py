...
class ADB:
    ...
    def get_game_version(self, game_type):
        ...
        result = self.exec_adb_cmd(
            f'shell "pm dump {package_name} | grep versionName"', each_timeout=5
        )
...
class AsstProxy:
    ...
    def __init__(self, id, last_logger: logging.Logger, device: Device, asst_callback: Asst.CallBackType) -> None:  # type: ignore
        ...
        self.userdir: pathlib.Path = (
            var.maa_usrdir_path / convert_str_to_legal_filename_windows(self._proxy_id)
        )
    ...
    def load_res(self, client_type: Optional[Union[str, None]] = None):
        ...
        self._logger.debug(
            f"Start to load asst resource and lib from incremental path {incr}"
        )
        if not try_run(
            Asst.load, (var.maa_env, incr, self.userdir), 2, 5, self._logger
        )[0]:
    ...
    def connect(self):
        ...
        for tried_time in range(max_try_time):
            ...
            if self.asst.connect(
                self.device._adb, self.device.addr, self.device.config_type
            ):
    ...
    def run_maatask(self, maatask, time_remain) -> "MaataskRunResult":
        ...
        for i in range(max_try_time):
            ...
            try:
                ...
                self._logger.debug(
                    f'current_maatask_status={self.status["current_maatask_status"]}'
                )
                if self.status["current_maatask_status"][0] == Message.TaskChainError:
                    if type == "StartUp":
                        self.device.adb.exec_adb_cmd(
                            f'shell am force-stop {arknights_package_name[self.device.current_status["server"]]}'
                        )
                    continue
                elif (
                    self.status["current_maatask_status"][0] == Message.TaskChainStopped
                ):
                    break
                else:
                    if type == "Fight":
                ...
                self._logger.info(
                    f"Maatask {type} {i+1}st/{max_try_time}max trying failed: {e}"
                )
        ...
        if succeed:
            self._logger.info(
                f"Maatask {type} ended successfully beacuse of {reason_str}"
            )
        else:
            self._logger.warning(
                f"Maatask {type} ended in failure beacuse of {reason_str}"
            )
...
class MaataskRunResult:
    ...
    def __init__(self, type, succeed, reason, tried_times, time_remain) -> None:
        self.type = type
        self.exec_result = MaataskRunResult.MaataskExecResult(
            succeed, reason, tried_times
        )
...
class ArknightsAPI:
    ...
    def get_newest_version() -> str:
        ...
        return (
            ArknightsAPI.get_newest_apk_link()
            .split("/")[-1]
            .replace(".apk", "")
            .split("-")[-1]
        )
...
class BiligameAPI:
    def get_newest_version() -> str:
        """return version"""
        return requests.get(
            "https://line1-h5-pc-api.biligame.com/game/detail/content?game_base_id=101772",
            timeout=10,
        ).json()["data"]["android_version"]

    def get_newest_apk_link() -> str:
        return requests.get(
            "https://line1-h5-pc-api.biligame.com/game/detail/gameinfo?game_base_id=101772",
            timeout=10,
        ).json()["data"]["android_download_link"]


class QooAppAPI:
    def get_newest_version(client_type) -> str:
        """return version"""
        id_list = {"YoStarJP": 7117, "YoStarEN": 9404, "YoStarKR": 9419, "txwy": 23510}
        id = id_list[client_type]
        ua = UserAgent().random
        html = requests.get(
            f"https://apps.qqaoop.com/app/{id}", headers={"User-Agent": ua}, timeout=10
        ).text
