...
def asst_callback(msg, details, arg):
    try:
        ...
        logger.error(
            f"An unexpected error was occured when receiving callback: {e}",
            exc_info=True,
        )
    ...
    try:
        ...
        if device.current_status["server"] != client_type:
            if device.current_status["server"]:
                device.adb.exec_adb_cmd(
                    f'shell am force-stop {arknights_package_name[device.current_status["server"]]}'
                )
        ...
        def update():
            ...
            if getupdate_support_info[client_type]:
                ...
                try:
                    ...
                    logger.warning(
                        f"An unexpected error was occured when getting update: {e}"
                    )
                ...
                if need_update:
                    if update_support_info[client_type]:
                        try:
                            ...
                            download_to = var.cache_path / convert_str_to_legal_filename_windows(
                                f"arknights_{client_type}_{newest}_{int(time.time())}.apk"
                            )
                            ...
                            logger.info(
                                "Arknights client has been successfully updated"
                            )
                        except Exception as e:
                            raise Exception(
                                f"The newest version is {newest} but the local version is {local}. When updating, an error occured: {e}"
                            )
                    else:
                        raise Exception(
                            f"The newest version is {newest} but the local version is {local}. The task cannot be run"
                        )
        ...
        for maatask in task["task"]:
            ...
            if remain_time > 0:
                if execute:
                    ...
                    result_maatasks.append(
                        MaataskRunResult(
                            maatask_name,
                            False,
                            [f"Skipped: disabled by {execute_disabled_by}"],
                            0,
                            0,
                        )
                    )
                ...
                result_maatasks.append(
                    MaataskRunResult(maatask_name, False, ["LackTime"], 0, 0)
                )
