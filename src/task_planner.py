...
def choice_stage(server, preference_dict: dict):
    stages_in_limit_list = [
        cp
        for cp in preference_dict
        if cp.rsplit("-", 1)[0] in arknights_stage_opening_time
    ]
    stages_outof_limit_list = [
        cp
        for cp in preference_dict
        if not cp.rsplit("-", 1)[0] in arknights_stage_opening_time
    ]
