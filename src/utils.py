...
def get_config_templates():
    ...
    for file in walk_dir(var.config_path):
        if file.startswith("template_") and (
            file.endswith(".yaml") or file.endswith(".yml")
        ):
            result[
                file.replace(".yml", "").replace(".yaml", "").replace("template_", "")
            ] = read_yaml(var.config_path / file)
    return result
...
def parse_arg():
    ...
    subparser_run = subparsers.add_parser(
        "run", help="Start running MAA according to config. "
    )
...
def get_logging_handlers():
    file_level, console_level = logging.DEBUG, (
        logging.DEBUG if var.verbose else logging.INFO
    )
...
def prase_MuMuVMMHeadless_commandline(headless_pid) -> dict:
    ...
    parsed, unknown = parser.parse_known_args(
        get_process_command_line(headless_pid)[1:]
    )
    return {"index": parsed.comment.split("-")[-1]}
...
def prase_MuMuPlayer_commandline(player_pid) -> dict:
    ...
    return {"index": parsed.v if parsed.v else "0"}
...
def get_MuMuPlayer_by_MuMuVMMHeadless(headless_pid) -> int | None:
    ...
    player_pids = [
        p
        for p in get_pids_by_process_name("MuMuPlayer.exe")
        if prase_MuMuPlayer_commandline(p)["index"] == index
    ]
    if player_pids:
...
def in_game_time(time, server="Official"):
    ...
    return (
        (time.astimezone(timezone.utc) - timedelta(hours=4))
...
def update_nav():
    ...
    last_update_time_file_server = (
        "https://ota.maa.plus/MaaAssistantArknights/api/lastUpdateTime.json"
    )
    ...
    last_update_time_content_server = requests.get(
        last_update_time_file_server, timeout=10
    ).content
    ...
    logging.debug(
        f"Tasks resource last update time is {last_update_time_local} and the data on server is {last_update_time_server}. need to update is {need_update}"
    )
    ...
    if need_update:
        ...
        write_file(
            ota_tasks_path,
            requests.get(ota_tasks_url, timeout=10).content.decode("utf-8"),
        )
        logging.debug(f"Asst tasks updated")

        write_file(
            last_update_time_file_local, last_update_time_content_server.decode("utf-8")
        )
